...
const tagColor = (i, isDark) =>
  isDark
    ? DARK_TAG_COLORS[i % DARK_TAG_COLORS.length]
    : LIGHT_TAG_COLORS[i % LIGHT_TAG_COLORS.length];
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        ...
        <p
          className={`text-sm line-clamp-2 leading-relaxed ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}
        >
        ...
        {(article.article_author?.user_name ||
          ...
          <p
            className={`text-xs mt-2 ${isDarkMode ? "text-zinc-500" : "text-gray-400"}`}
          >
            {article.article_author?.user_name && (
              <span>
                by{" "}
            ...
            {article.article_subject?.subject_name && (
              <span className="ml-2">
                in{" "}
                <span
                  className={`font-semibold ${isDarkMode ? "text-zinc-300" : "text-black"}`}
                >
                  {article.article_subject.subject_name}
                </span>
              </span>
      ...
      >
        <div
          className={`flex items-center gap-1.5 text-xs font-medium md:mb-4 ${isDarkMode ? "text-zinc-500" : "text-gray-400"}`}
        >
          <Clock size={13} />
          {readTime} min read
        </div>
            ...
            <>
              ...
              <button
                onClick={(e) => {
                  ...
                  if (
                    window.confirm(
                      `Are you sure you want to delete "${article.article_title}"?`,
                    )
                  ) {
...
const EmptyState = ({ isDarkMode }) => (
  <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto py-20">
    ...
    >
      <FileText
        size={32}
        className={isDarkMode ? "text-zinc-600" : "text-gray-300"}
      />
    </div>
    <h2
      className={`text-xl font-bold mb-2 ${isDarkMode ? "text-zinc-200" : "text-gray-800"}`}
    >
      No Articles Found
    </h2>
    <p className={`text-sm ${isDarkMode ? "text-zinc-500" : "text-gray-500"}`}>
      Select a subject from the sidebar to view its articles, or check back
...
// ── Main Body ───────────────────────────────────────────────────────────────
const MainBody = ({ onEditArticle }) => {
  const { articles, isFetchingArticles, readArticle, deleteArticle } =
    useArticleStore();
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h1
            className={`text-2xl font-bold flex items-center gap-3 ${isDarkMode ? "text-zinc-100" : "text-gray-900"}`}
          >
            <BookOpen
          ...
          <p
            className={`text-sm mt-0.5 ${isDarkMode ? "text-zinc-500" : "text-gray-400"}`}
          >
            {articles.length > 0
        ...
        <div className="flex items-center gap-3">
          <div
            className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl border text-sm font-medium transition-colors
            ${
              isDarkMode
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-8">
        {isFetchingArticles ? (
          <div className="h-full flex flex-col items-center justify-center gap-4">
            <Loader2
              size={38}
              className={`animate-spin ${isDarkMode ? "text-zinc-400" : "text-black"}`}
            />
            <p
              className={`text-sm font-medium animate-pulse ${isDarkMode ? "text-zinc-500" : "text-gray-500"}`}
            >
              Fetching articles…
            </p>
          </div>
